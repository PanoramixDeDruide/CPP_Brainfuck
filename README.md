...
5. Run ./run.sh
6. (Optional): you can look at the interpreter's internals while it's running by running `watch -n1 cat one.h` and/or `watch -n1 cat two.h` from (a) separate terminal(s). (`one.h` and `two.h` located in `brainfuck/` during interpretation)
7. Wait for it to complete. WARNING: It took my AMD Ryzen 5 5600X over a week to buffer just six characters from the visualized Mandelbrot set. Output will be presented to you in your terminal once done.
